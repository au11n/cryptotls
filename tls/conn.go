...
type Conn struct {
	...
	// FORK: embedded atLeastReader to avoid per-Read heap allocation.
	// Standard crypto/tls creates &atLeastReader{} on each readFromUntil call,
	// which escapes to heap via io.Reader interface (~2 allocs per TLS record).
	atLeast atLeastReader
}
...
//
// FORK: uses embedded c.atLeast instead of allocating &atLeastReader{} on each call.
func (c *Conn) readFromUntil(r io.Reader, n int) error {
	...
	c.atLeast.R = r
	c.atLeast.N = int64(needs)
	_, err := c.rawInput.ReadFrom(&c.atLeast)
	return err
}
